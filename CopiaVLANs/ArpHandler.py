...
class ArpHandler(app_manager.RyuApp):
    ...
    def set_shortest_path(self, ip_src, ip_dst, src_dpid, dst_dpid,
        ...
        Instala en los switches el camino más corto (ECMP round-robin) entre
        src_dpid y dst_dpid para el par (ip_src, ip_dst), diferenciando por VLAN.
        No se hace pop_vlan: las subinterfaces en los hosts se encargarán de la etiqueta.
        ...
        # 4) Si el host origen y destino están en el mismo switch
        if len(path) == 1:
        ...
        # 6) Instalar flujo en el switch destino (sin pop_vlan)
        dst_dp = self.get_datapath(dst_dpid)
        parser_dst = dst_dp.ofproto_parser
        actions = pre_actions + [parser_dst.OFPActionOutput(to_port_no)]
        self.add_flow(dst_dp, 10, match, actions)

        # 7) Calcular el puerto de salida en el primer salto
        u, v = path[0], path[1]
        edges = list(self.graph.get_edge_data(u, v).items())
        if not edges:
            self.logger.warning(f"No hay enlace físico {u}→{v}")
    ...
    def add_flow(self, dp, p, match, actions, idle_timeout=0, hard_timeout=0):
        ...
        # self.logger.info(f"[FLOW INSTALL] switch={dp.id} priority={p} match={match} actions={actions}")
        dp.send_msg(mod)
