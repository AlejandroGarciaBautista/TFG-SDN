...
def create_spine_leaf_topology(spine_switches, leaf_switches, hosts_per_leaf, link_bandwidth, redundancy, controller_ip):
    ...
    spines = [net.addSwitch(f"spine{i+1}", dpid=f"00000000000{i+1}", protocols="OpenFlow13") for i in range(spine_switches)]
    leaves = [net.addSwitch(f"leaf{i+1}", dpid=f"00000000000{spine_switches + i+1}", protocols="OpenFlow13") for i in range(leaf_switches)]
    ...
    # 1️⃣ Regla para permitir el tráfico ARP
    for i in range(1, spine_switches + leaf_switches + 1):
    ...
    # 2️⃣ Regla para permitir el reenvío de paquetes LLDP
    for i in range(1, spine_switches + leaf_switches + 1):
    ...
    # 3️⃣ Permitir el reenvío de tráfico desconocido
    for i in range(1, spine_switches + leaf_switches + 1):
